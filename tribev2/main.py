    ...
    def get_loaders(
        ...
        for timeline_name, timeline in events.groupby("timeline"):
            ...
            if "split" in timeline.columns:
                #print("Hello from frist case  :if split in timeline.columns:")
                splits = timeline.split.dropna().unique()
                if len(splits) > 1:
                    #print("Hello from frist case  :if split in timeline.columns:if len(splits) > 1:")
                ...
                #print("Hello from second case  of  :if split not in timeline.columns:split = all")
                split = "all"
            dummy_event = {
        ...
        for split in splits:
            ...
            print(f"[DEBUG] events[sel] type counts:\n{events[sel].type.value_counts()}")
...
class TribeExperiment(BaseExperiment):
    ...
    test_only: bool = False # for inference mode
